...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFpcGtzdHZtbGJqdHZtYm95eW92Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUzMjQ2NDAsImV4cCI6MjA5MDkwMDY0MH0.ZWA-TlRMRXIeHMCaemv-YqmWL713hZnexHxUDHwVz5c';
...
function parseRows(data) {
  ...
  const find = (row, ...keys) => {
    for (const k of keys) {
      const found = cols.find(c => c.toLowerCase().replace(/\s/g, '').includes(k));
      if (found) return (row[found] || '').trim();
    }
    return '';
  };
  return data.map(r => ({
    theme: find(r, 'thème', 'theme'),
    fr: find(r, 'français', 'francais', 'fr', 'mot'),
    genre: find(r, 'genre'),
    itSg: find(r, 'sg', 'italien', 'it'),
    itPl: find(r, 'pl'),
    tip: find(r, 'astuce', 'tip'),
  }));
}

async function mergeImport(rows) {
  const existing = new Set(cards.map(c => c.fr.toLowerCase().trim()));
  const toAdd = rows.filter(r => r.fr && r.itSg && !existing.has(r.fr.toLowerCase().trim()));
  if (!toAdd.length) {
